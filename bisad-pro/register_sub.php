<?php 
  ...
  function business_registration($cnct){
    $region = $city = $subcity = $sefer = $additional_detail = $company = $category = $business_id = $profile = "";
      ...
      if(isset($_POST['submit']) && !empty($_POST['region']) && !empty($_POST['city']) && !empty($_POST['subcity']) && !empty($_POST['add_detail']) && !empty($_POST['company']) && !empty($_POST['catagory']) && !empty($_POST['detail'])){
